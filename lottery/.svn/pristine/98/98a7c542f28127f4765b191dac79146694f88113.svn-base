...
public class KafkaConsumer implements MessageListener<String, SampleMessage> {
    ...
    public void test(JSONObject data) {
    	...
    	try {
    		textMessage = messageUpdate(data);
    	} catch (Exception e1) {
    		logger.error("处理消息发生错误" + data.toString());
    	}
    	handler.broadcastByGame(textMessage, Constants.LOTTERY_TYPE);
    }
...
//                    String hash = jsonObject.get("id").toString();
    ...
    public void getPrize(List<Integer> num, List<String> block, Date date) {
        ...
        threadPoolTaskExecutor.execute(new Runnable() {
            public void run() {
                PublicMethod.senPrizeMsg(records, datetime, userBetService, userBetScoreService,
                		accountService, logger, lotteryUserService, handler, gameService,userTransactionScoreRecordsService);
            }
        });
    }
}