...
public class TigerOpenConsumer implements MessageListener<String, SampleMessage> {
	...
	public void onMessage(ConsumerRecord<String, SampleMessage> data) {
		...
		try {
			...
			String key = "block_num_t";
			Object object = redisService.getCache(key);
			if (null != object) {
				int lastNum = Integer.parseInt(object.toString());
				if (i > lastNum) {
					redisService.setCache(key, block_num);
				} else {
					return;
				}
			} else {
		...
		for (TigerRoom room : MainData.tigerRooms) {
			...
			case wash: {
				...
				if (now - lastUpdate >= Constants.TIGER_WASH_TIME) {
					...
					handler.broadcastToSome(room.getHeads(), new TextMessage(change));
					if (roomId == 1001) {
						// System.out.println("房间状态变为：" +
						// TigerTimeState.getEnumById(room.getState()));
					}
				}
			}
				break;
			...
			case open: {
				...
				if (room.getBlockOpen().getBlock_number() + x == block_number && room.getBlockLong() == null) {
					...
					if (card == null) {
						_log.error("重大错误，无法计算开奖花色点数，请修改代码！");
						return;
					}
					...
					bo.put("hua", card[0]);
					bo.put("p", card[1]);
				} else if (room.getBlockOpen().getBlock_number() + y == block_number && room.getBlockHu() == null) {
					// 虎牌区块出现
					...
					try {
						// 存储开奖历史记录
						TigerHistory th = new TigerHistory(roomId, qid, room.getCardLong()[2], room.getCardHu()[2],
								room.getBlockOpen().getBlock_time());
					...
					if (type == 0) {
						...
						// 数据库查出本房间本期投注记录（积分投注）
						List<TigerPutRecord2> updateRecords = tigerService.getRecordsWhenOpen2(roomId, qid);
						// 更新玩家投注记录
						for (int i = 0; i < updateRecords.size(); i++) {
							TigerPutRecord2 record = updateRecords.get(i);
							// 使得每条记录开奖时间不同，便于查询
							record.setOpenTimeSpt(room.getBlockOpen().getBlock_time() + i);
							String userId = record.getUserId();
							// 更新中奖金额
							if (goodOpts.contains(record.getOpt())) {
								record.setStatus(2);
								...
								// 更新到数据库
								tigerService.updateRecord2(record);
								...
								map2.put("qid", qid);
								map2.put("money", gainMoney);// 中奖金额
								map2.put("time", record.getOpenTimeSpt());
								String json2 = JsonUtil.buildJson(MsgCode.TIGER_PUT_SHOW, map2);
								handler.broadcastToSome(room.getHeads(), new TextMessage(json2));
							} else {
								record.setStatus(1);
							...
							map3.put("roomId", roomId);
							map3.put("qid", qid);
							map3.put("time", record.getOpenTimeSpt());
							map3.put("opt", record.getOpt());
							map3.put("put", record.getPutMoney());
							map3.put("gain", record.getGainMoney());
							String json3 = JsonUtil.buildJson(MsgCode.TIGER_MY_PUT_PUSH, map3);
							handler.sendMessageToUser(userId, new TextMessage(json3));
						}
						...
						Map<String, Map<Integer, List<BigDecimal>>> sendMap = new HashMap<>();
						Set<String> heads = room.getHeads();
						for (TigerPutRecord2 record : updateRecords) {
							if (record.getStatus() != 2) {// 排除未中奖的记录
								continue;
							}
							String userId = record.getUserId();
						...
						_log.error("房间没有type，无法更新投注记录（开奖）！");
					}
				}
				...
				if (!room.isDone() && room.getCardHu() != null) {
					if (type == 0) {
							...
							public void run() {
								...
								for (TigerPutRecord record : updateRecords) {
									...
									if (!accountService.checkTxId(txId)) {
										updater.setStatus(-2);
										tigerService.updateRecord(updater);
										_log.error("发奖时查不到该hash，不发奖！ txId:" + txId);
										continue;
									}
									...
									// 发奖转账
									boolean success = accountService.plusBalanceFromTiger(userId,
											putMoney.add(gainMoney));
									if (success) {
										updater.setStatus(3);
										tigerService.updateRecord(updater);
										_log.debug("发奖成功！ userId: " + userId + ", money: " + putMoney.add(gainMoney));
										// 流水记录
										EosFlow ef = new EosFlow(userId, Constants.GAME_TYPE_TIGER, 1, 1, roomId,
												Long.valueOf(record.getQid()), putMoney.add(gainMoney), null);
								...
								if (poolBalance != null) {
									...
									handler.broadcastByGame(new TextMessage(json4), Constants.TIGER_TYPE);
								}
							}
						});
					} else if (type == 1) {
						...
						threadPoolTaskExecutor.execute(new Runnable() {
							...
							public void run() {
								// 数据库查出本房间本期需要发奖的投注记录
								List<TigerPutRecord2> updateRecords = tigerService.getRecordsWhenSend2(roomId, qid);
								if (updateRecords.size() == 0) {
									return;
								}
								for (TigerPutRecord2 record : updateRecords) {
									...
									BigDecimal putMoney = record.getPutMoney();
									BigDecimal gainMoney = record.getGainMoney();
									BigDecimal winMoney = putMoney.add(gainMoney);
									// 积分发奖
									boolean success = lotteryUserService.plusScoreFromTiger(uid, winMoney);
									if (success) {
										record.setStatus(3);
										tigerService.updateRecord2(record);
										_log.debug("积分发奖成功！ userId: " + userId + ", money: " + winMoney);
										// 通知用户余额变动
										if (room.getHeads().contains(userId)) {
											PublicMethod.sendScoreUserBalance(uid, Constants.TIGER_TYPE,
													lotteryUserService, handler);
										}
									} else {
										record.setStatus(-1);
										tigerService.updateRecord2(record);
										_log.error("积分发奖失败! userId: " + userId + ", money: " + putMoney.add(gainMoney));
									}
								}
						...
						_log.error("房间没有type，无法更新投注记录（发奖）！");
					}
					// 发奖结束，标记
					room.setDone(true);
				}
